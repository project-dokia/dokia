...
public class MailJavaSender {
	//cria as propriedades necess�rias para o envio de email
	...
	public void senderMailFull(final MailJava mail) throws
	UnsupportedEncodingException, MessagingException {
		Properties props = new Properties();
		props.setProperty("mail.transport.protocol", "smtp");
		props.setProperty("mail.host", mail.getSmtpHostMail());
...
//		props.put("mail.smtp.starttls.enable", "true"); 
		props.put("mail.smtp.localhost", "127.0.0.1");//local smtp, erro no mac
		//classe anonima que realiza a autentica��o
		//do usuario no servidor de email.
		Authenticator auth = new Authenticator() {
			@Override
		...
		/** Mails TO */
		for (Map.Entry<String, String> map1 : mail.getToMailsUsers().entrySet()) {
			msg.addRecipient(Message.RecipientType.TO,
					new InternetAddress(map1.getKey(), map1.getValue())
			);
		}
		/** Mails CC */
		for (Map.Entry<String, String> map2 : mail.getCcMailsUsers().entrySet()) {
			msg.addRecipient(Message.RecipientType.CC,
					new InternetAddress(map2.getKey(), map2.getValue())
			);
		}
		/** Mails BCC */
		for (Map.Entry<String, String> map3 : mail.getBccMailsUsers().entrySet()) {
			msg.addRecipient(Message.RecipientType.BCC,
					new InternetAddress(map3.getKey(), map3.getValue())
			);
		}
		// Adiciona um Assunto a Mensagem
		msg.setSubject(mail.getSubjectMail());
		...
		System.out.println("mail.getSmtpHostMail="+mail.getSmtpHostMail());
		// Envia a Mensagem
		Transport.send(msg);
	}
}